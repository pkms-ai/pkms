...
async def submit_content(submission: ContentSubmission):
    try:
        ...
        if classified_content is None or classified_content.content_type == ContentType.UNKNOWN:
            return JSONResponse(status_code=400, content={"detail": "Failed to classify the content"})
        ...
        for attempt in range(settings.RETRY_ATTEMPTS):
            try:
                ...
                if attempt == settings.RETRY_ATTEMPTS - 1:
                    return JSONResponse(
                        status_code=503,
                        content={"detail": f"Queue service unavailable after {settings.RETRY_ATTEMPTS} attempts. Please try again later."},
                    )
